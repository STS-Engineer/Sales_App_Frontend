...
function getActionBadgeClass(action) {
  ...
  if (a.includes("approve") || a.includes("accept") || a.includes("complete")) {
    return "border-mint/40 bg-mint/10 text-mint";
  }
  if (a.includes("create") || a.includes("submit") || a.includes("new")) {
    return "border-tide/30 bg-tide/10 text-tide";
  }
  if (a.includes("update") || a.includes("edit") || a.includes("revis")) {
...
export default function Logger() {
  ...
  const filteredLogs = useMemo(() => {
    ...
    return logs.filter((log) =>
      [log.systematic_rfq_id, log.action, log.performed_by].some((value) =>
  ...
  const stats = useMemo(() => {
    const rfqs = new Set(logs.map((log) => log.systematic_rfq_id).filter(Boolean));
    const users = new Set(logs.map((log) => log.performed_by).filter(Boolean));
    return { total: logs.length, rfqs: rfqs.size, users: users.size };
  }, [logs]);
      ...
      <div className="w-full space-y-6">
          ...
          <div className="flex flex-wrap items-center gap-3">
            <div className="relative w-full sm:w-72">
              <input
                ...
                type="search"
                placeholder="Search RFQ, action, user..."
                value={searchTerm}
        ...
        <div className="grid gap-4 sm:grid-cols-3">
          ...
          <div className="relative overflow-hidden rounded-2xl border border-slate-200/70 bg-white/95 p-4 shadow-soft">
            ...
            <p className="pl-2 mt-1 font-display text-3xl text-ink">{stats.users}</p>
          </div>
        </div>
              ...
              <table className="min-w-[960px] w-full text-left text-sm">
                <thead className="sticky top-0 bg-slate-100/90 text-xs uppercase tracking-widest text-slate-500 backdrop-blur">
                  <tr>
                    ...
                    <th className="px-6 py-4 whitespace-nowrap">Performed by</th>
                  </tr>
                </thead>
                <tbody>
